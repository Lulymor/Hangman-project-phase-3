...
def user_login_signup():
    ...
    global add_username
    global add_password
    has_username = input('\n Do you have a username? "y" = yes "n" = no \n')
    if has_username == 'y':
        print('\n Log in')
        ...
        if create_user == 'y':
            print('\n Sign up')
            add_username = input('\n Pick a username: \n')
            add_password = input(f'\n Pick a password for {add_username} \n')
...
def pick_category():
    ...
    if category == "1":
        ...
        print('\n You Picked Programmers by last name')
        main()
    elif category == '4':
        category_picked = lighthousesdb
        print('\n You picked Light Houses')
        main()
    else:
        print('\n Pick a category by number 1 to 4')
        exit()
     ...
     \   // _ \ | | | |   \ \/  \/ /  | || '_ \ | |
      ...
      |_| \___/  \__,_|     \/  \/    |_||_| |_|(_)
        ...
        if usersdb.search_user(f'{username}') == True:
            ...
            print(f'\n {username} your new score is: {new_score}')
            play_loop()
